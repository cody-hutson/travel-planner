...
scripts/publish-trip-site.sh confirm   trips/<destination>-<year>            # record an approval (terminal only)
...
**Group approval.** When a republish would change the published plan, `update` stops until the change is approved. By default the organizer approves it at a terminal with `scripts/publish-trip-site.sh confirm trips/<destination>-<year>`. To make a change wait for named travellers instead, declare them with `/trip-record .approvers` — all of them must approve unless you set `--threshold` — and run the same `confirm`: it prints a one-line approval code to share in the group's thread with the change summary, then records each traveller's reply, one at a time, as your statement on their behalf. The published site shows how many approvals you recorded and the plan's code, never who approved. What that count proves, and what it does not, is stated in [`SECURITY.md`](SECURITY.md).
