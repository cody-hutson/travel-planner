...
| **Person erasure** | A person asked to be deleted, and their data must go from the store and from every trip that copied it | Show the reach before anything changes, confirm by typing the record's id at a terminal, then substitute their identifying values everywhere the report named and delete the record. Reaches archived trips; regenerates nothing; never echoes the erased value. | "Delete Dana's record entirely", "She's asked to be erased" | `/trip-record erase` |
