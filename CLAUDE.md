...
|------|--------|--------|---------|---------|
| **Orientation** | User is starting a session, or asks where the trip stands and what to do next | **INFERENCE-ADMITTED** — Resolve the active trip and its mode; state what is available and what comes next. Read-only — it mutates nothing. | "Where are we?", "What's the status?", "What can I do next?" | `/trip status` |
...
| **Destination ideation** | User is exploring where to go and no destination is chosen yet | **DECLARED-INTENT** — Turns the group's leanings into a ranked shortlist (`outputs/destination-shortlist.md`) for the group to decide from. | "Where should we go?", "Give us some options for spring" | `/trip ideas` |
| **Targeted research** | User wants new options or deeper research on a specific topic | **DECLARED-INTENT** — The relevant spoke agent produces targeted research on the topic and appends it to the existing output file. | "Find more dinner options near Bairro Alto", "What indoor activities exist near the hotel?" | `/trip research` |
| **Planning change** | User wants to change the itinerary structure (swap days, add a day trip, reschedule) | **DECLARED-INTENT** — Mode notes in trip-context.md are updated. Only affected agents re-run. Hub patches itinerary. Only `planned` events change freely; `locked`/`firmed` events are preserved unless the user names them (see Key Rules → per-event status). | "Swap Day 3 and Day 4", "Replace the afternoon on Day 5 with something indoor" | `/trip replan` |
| **Day resequencing** | User wants the same selections kept but the days reordered | **DECLARED-INTENT** — Mode advances to RESEQUENCING and is announced — see Modes for what runs. Only `planned` events move. | "Reorder the days", "Same plan, different day order" | `/trip reorder` |
| **Full pipeline** | User wants the initial plan built or a full re-plan | **DECLARED-INTENT** — Runs the full agent pipeline (enrichment → spokes → hub → validator) | "Build the itinerary", "Start fresh on the plan" | `/trip plan` |
| **Ambiguous — reshape the plan** | User wants the itinerary changed and has not said how much of what is there survives | State the distinction and let the user pick: `replan` keeps the research and rebuilds the affected structure, `reorder` keeps the same selections and moves only the days, `plan` rebuilds from enrichment down. Propose the choice; never resolve it here. | "Change the plan", "Can we redo this?" | AMBIGUOUS: `/trip replan` · `/trip reorder` · `/trip plan` |
| **Plan audit** | User wants the existing plan checked without changing it | **DECLARED-INTENT** — Runs the Validator alone against the current itinerary and reports findings. No spokes, no hub, no edits. | "Does the plan hold up?", "Check the itinerary for problems" | `/trip check` |
| **Site generation** | User wants the travel site built or rebuilt | **DECLARED-INTENT** — See Travel Site Generation section | "Build the site", "Create the travel page" | `/trip site` |
| **Artifact validation** | User wants their own trip's files checked against the declared schema | **INFERENCE-ADMITTED** — Runs the schema check over the resolved trip's artifacts and reports per-artifact, per-field failures. Read-only, no network, publishes nothing. | "Do my trip files still parse?", "Check the trip data" | `/trip schema` |
| **Ambiguous — check what** | User asks for the trip to be checked and has not said whether they mean the plan or the files it is made of | State the distinction and let the user pick: `check` audits the plan's substance against the constraints, `schema` validates the trip files' structure against the declared schemas. Propose the choice; never resolve it here. | "Check the trip", "Does this still hold up?" | AMBIGUOUS: `/trip check` · `/trip schema` |
| **Publish** | User wants to push to GitHub | **DECLARED-INTENT** — See Publishing section | "Publish this", "Push to GitHub" | `/trip-publish update` |
| **New trip** | User wants to start a trip that does not exist yet | **DECLARED-INTENT** — Scaffold the trip directory and its members and set the starting mode from what the user stated. Resumes, repairing only what is missing, when the slug already exists. | "Start planning Lisbon 2027", "New trip to Tokyo" | `/trip-new` |
| **Traveler profile** | User wants to create or update a traveler's own profile | **DECLARED-INTENT** — Create the profile that does not exist, edit the one that does. Never invent a field — an unanswered field is a skipped field. | "Add Dana's profile", "Update my dietary needs" | `/trip-record profile` |
| **Third-party traveler** | A party member who will never file a profile, whose needs the operator supplies | **DECLARED-INTENT** — Record the third-party entry and reconcile the derived traveler model. | "Add Mom — she can't do stairs", "Record Sam's constraints for him" | `/trip-record person` |
| **Ambiguous — whose answers** | User wants somebody's needs recorded and has not said who supplies the answers | State the distinction and let the user pick: `profile` is for a traveller who fills in their own form, `person` is for a party member who never will and whose needs the operator supplies. Propose the choice; never resolve it here. | "Add Mom", "Record what Sam needs" | AMBIGUOUS: `/trip-record profile` · `/trip-record person` |
| **Ambiguous — a note or a person** | User states a constraint about a party member and has not said whether it is a passing note on this trip or that person's standing need | State the distinction and let the user pick: the lightest-weight action records it in `trip-context.md` and creates nothing, `person` opens a third-party entry the derived traveler model reconciles and carries forward. Propose the choice; never resolve it here. | "Mom can't do stairs", "Sam needs a rest day" | AMBIGUOUS: EXCLUDED: lightest-weight-action · `/trip-record person` |
| **Roster reconciliation** | Traveler files changed and the derived model is behind them | **DECLARED-INTENT** — Enrichment alone, in its reconciler role — never the research role. | "Re-sync the traveler model", "The profiles changed" | `/trip-record travelers` |
| **Destination hand-off** | The group has chosen a destination from the shortlist | **DECLARED-INTENT** — Write the chosen destination into `trip-context.md`. The ideation agent never writes it itself. | "We picked Lisbon", "Go with option 2" | `/trip-record destination` |
| **Mode change** | The trip has moved to a different phase, or the mode is unset | **DECLARED-INTENT** — Replace `Current mode` and `Mode notes` in one act, echoing the outgoing notes. This is the remedy an unset mode's refusal names. | "We've booked flights", "Set the mode to ITERATION" | `/trip-record mode` |
...
| **Person reference** | A traveler on this trip is the same person as a record already in the people library | **DECLARED-INTENT** — Point this trip's traveler file at that record — one frontmatter field and nothing else. The record is read to confirm the id resolves and to name who it resolves to. | "Dana is the same Dana as last time", "Link Alex to their record" | `/trip-record link` |
| **Reference removal** | A traveler's durable record should stop being read for this trip | **DECLARED-INTENT** — Remove that one frontmatter field, echoing it first, and state plainly that the person record itself survives. Writes no marker of any kind. | "Stop reading Alex's durable record here", "Take Dana's link off this trip" | `/trip-record unlink` |
| **Override promotion** | A trip-local answer turns out to be durable and belongs in the person record | **DECLARED-INTENT** — Move **one** named field from the traveler's file into the record it references, confirmed against an echoed outgoing → incoming pair. Explicit and typed, never inferred. | "That allergy is permanent — put it on her record", "Make Dana's mobility note durable" | `/trip-record promote` |
| **Person erasure** | A person asked to be deleted, and their data must go from the store and from every trip that copied it | **DECLARED-INTENT** — Show the reach before anything changes; the prompt names the record's display name and id, and the operator confirms by typing **the id** at a terminal; then substitute their identifying values everywhere the report named and delete the record. Reaches archived trips; regenerates nothing; echoes the name once at the prompt so a mis-targeted run is visible, and never echoes an erased value afterwards. | "Delete Dana's record entirely", "She's asked to be erased" | `/trip-record erase` |
| **Profile extraction** | A traveller's existing profile holds durable facts that belong in a person record they do not yet have | **DECLARED-INTENT** — Create the record from that file's own answered fields and point the file at it. **Copies — never moves**: no byte of the profile's body changes, so it keeps working either way. One file per invocation, confirmed against a preview that names every answered field and echoes no value but the display name. | "Make Alex a person record", "Turn Dana's profile into a library record" | `/trip-record extract` |
| **Ambiguous — link or build** | User wants a traveller joined to the people library and has not said whether the durable record already exists | State the distinction and let the user pick: `link` points the profile at a record that already exists, `extract` builds one from the profile's own answered fields and then points at it. Propose the choice; never resolve it here. | "Link Alex to their record", "Make Alex a person record" | AMBIGUOUS: `/trip-record link` · `/trip-record extract` |
| **New reusable group** | The same people travel together often enough to be worth naming once | **DECLARED-INTENT** — Mint a group record holding a display name and an empty member list. Refuses when the normalized name already exists, offering three remedies rather than guessing. | "Make a group for the ski crew", "Name this set of people" | `/trip-record group-new` |
| **Group inventory** | User asks what groups exist, or who is in one | **INFERENCE-ADMITTED** — No argument lists the store — id, name, member count, and no member ids. An argument lists that group's members, each id resolved to a display name; one that no longer resolves reads *unknown*, never *no such person*. Writes nothing. | "What groups do I have?", "Who's in the ski crew?" | `/trip-record group-list` |
| **Group membership — add** | Someone joins a reusable group | **DECLARED-INTENT** — Append one member id. Refuses a duplicate and refuses an id that does not resolve — it **never creates a person record**, which is a different act with its own consent boundary. | "Add Dana to the ski crew", "Rhian's part of that group now" | `/trip-record group-add` |
| **Group membership — remove** | Someone leaves a reusable group | **DECLARED-INTENT** — Remove one member id, echoed verbatim first. Leaves the person's record untouched and leaves every trip that already expanded the group byte-identical. | "Take Sam out of the ski crew", "Dana's not in that group any more" | `/trip-record group-drop` |
| **Ambiguous — which roster** | User takes somebody out of a set of travellers and has not said which set | State the distinction and let the user pick: `group` edits this trip's own roster block, `group-drop` removes a member from a reusable group that outlives the trip. Propose the choice; never resolve it here. | "Sam's not coming", "Take Sam out of the ski crew" | AMBIGUOUS: `/trip-record group` · `/trip-record group-drop` |
| **Group deletion** | A reusable group is no longer worth keeping | **DECLARED-INTENT** — Delete the group record and nothing else — no person record, no trip. Confirms against the group's display name and member count, and the receipt prints the member ids so the group can be rebuilt. | "Delete the ski crew group", "Drop that group" | `/trip-record group-delete` |
| **Group expansion onto a trip** | A named group is travelling on this trip, and its members should be set up as a unit | **DECLARED-INTENT** — Put every member on the trip, each linked to their own record exactly as `link` would link them one at a time. Runs `link`'s consequence survey per member and shows one consolidated preview — a member whose link would change something is named by field label and left out. **Point-in-time: no trip file names the group**, so editing the group later leaves this trip byte-identical. | "Put the ski crew on this trip", "Expand that group onto Lisbon" | `/trip-record group-expand` |
| **Prior-visit lookup** | User asks whether a traveler has already been to this destination, or wants the depth field answered from what the trips already say | **INFERENCE-ADMITTED** — Resolve that person's other trips by inverting the trip-side reference, present them as candidates for a human to match, and offer a value only for an unanswered field. Writes nothing anywhere: the traveler's own answer is the only thing that lands in their file. Each kind of absence is reported as a different thing — a completed scan that found nothing, a traveler referencing no durable record at all, and an indeterminate read are three distinct outcomes — and none of them ever reads as *has not been here*. | "Has Dana been to Lisbon before?", "Have any of us been here already?" | `/trip-record history` |
| **Published inventory** | User asks what is published, across trips | **INFERENCE-ADMITTED** — Report the published sites the script resolves. Needs no trip resolved. | "What's published?", "Which trips have sites?" | `/trip-publish list` |
| **Temporary takedown** | User wants the site offline but the trip kept | **DECLARED-INTENT** — Disable Pages. The local tree is untouched and no marker is written. | "Take the site down for now", "Hide it temporarily" | `/trip-decommission temporary` |
| **Trip conclusion** | The trip is over and should be concluded | **DECLARED-INTENT** — The takedown, then the lifecycle marker, then a closing log entry — in that order, and the order is load-bearing. | "The trip's done", "Archive Lisbon 2026" | `/trip-decommission archive` |
| **Ambiguous — offline or concluded** | User wants the site taken down and has not said whether the trip itself is over | State the distinction and let the user pick: `temporary` disables Pages and leaves the trip active, `archive` takes the site down and then marks the trip concluded. Propose the choice; never resolve it here. | "Take the site down", "We're done with this one" | AMBIGUOUS: `/trip-decommission temporary` · `/trip-decommission archive` |
| **Trip reopen** | An archived trip needs to be worked on again | **DECLARED-INTENT** — Return `**Lifecycle:**` to `ACTIVE`, echoing the outgoing value. | "Reopen Lisbon", "I need to change the archived trip" | `/trip-decommission reopen` |

**The entry class, and what an ambiguity row does not carry.** Every ADDRESSED row's Action cell opens with its **entry class** — `**INFERENCE-ADMITTED**` or `**DECLARED-INTENT**`, and nothing else — then an em dash, then the action itself. The tokens are uppercase renderings of `inference-admitted` and of the declared-intent side in [`reference/adr/ADR-007-command-entry-point.md`](reference/adr/ADR-007-command-entry-point.md) § 1; they name nothing that record does not. **This table records the class, it does not decide it.** The single source is the verb's own section in its `skills/<name>/SKILL.md`, whose `**Reads:**` block has to declare each of writes nothing, dispatches no agent, and performs no act whose effect lands outside the trip's own files — and the boundary is fail-closed, so a block silent on any of them leaves its verb on the declared-intent side. An `AMBIGUOUS:` row carries **no** class, because a set is not an arm: a member naming a verb carries its class on that verb's own row, and a member naming a declared disposition carries none, because a disposition is not an arm either and an `EXCLUDED:` row carries no class. **A choice is proposed and never executed.** That is unconditional, and it is unconditional because [`ADR-007`](reference/adr/ADR-007-command-entry-point.md) § 3 states it that way: a set is how a row says an intent reaches more than one verb and is therefore rendered as a choice rather than resolved to one. Nothing qualifies it — not which side a member sits on, not how many members there are, not what this table happens to declare at any revision. A rule written with a condition invites the converse, and the converse here is resolving a set by inference, which § 3 does not admit.
...
| Nightlife | `agents/07-nightlife.md` | `outputs/nightlife-list.md` | User wants going-out research, or a full pipeline where a present traveler holds a nightlife/evening desire, or the trip contains at least one night a natural occasion would apply to (a weekend night, or a special occasion in trip-context.md's travel dates or calendar events) — the spoke resolves its own desire gate and writes a gate-result stub when nobody does |
...
| ITERATION | Existing plan, user wants changes — including a **disruption recovery**, triggered two ways: an event regressing `locked → planned` in `event-status.md` (a missed booking / cancelled hold) **or** a changed-profile delta (the enrichment agent's update signal that a traveler edited their file). | Only affected agents re-run. Hub patches itinerary. Validator re-checks changed days. Status honored: only `planned` events change freely; `locked`/`firmed` are preserved unless the user names them; `option` events stay alternatives. On a disruption recovery the hub runs its **equity-aware disruption recovery** (per-traveler loss distribution → prioritize hardest-hit → re-run affected engines, needs preserved → regroup gaps under a coherent theme), and the validator runs its **recovery-equity check** (losses not concentrated; needs still hold). |
| RESEQUENCING | Keep selections, reorder days | Scheduling re-runs. Hub resequences. Validator checks new day-of-week assignments, running a **full pass on all days after a resequence** — the sequence change may introduce a conflict on a day no venue moved on, so a changed-day narrowing would miss it. Only `planned` events move; `locked`/`firmed` are fixed anchors; `option` events are not promoted into primary slots. Where the resequence is itself **driven by a disruption** — a `locked → planned` regression re-opened an anchor — the hub runs its **equity-aware disruption recovery** so the new sequence rebalances the loss rather than merely reordering around the hole, and the validator runs its **recovery-equity check** against that recovery. The audit follows the recovery: wherever the hub may rebalance, the validator may audit it. |

**Horizon-awareness spans these modes rather than sitting in one of them.** Where the trip is near enough that a booking deadline is in play, the `days-to-trip-start` signal and the priority shift it carries are defined in `reference/replan-protocol.md`.

**The mode-gated behaviour register — the single declaration of which agent-side branches are gated on a mode.** A behaviour that branches on the mode is declared here, once, with the prompt that carries it, the modes that gate it, and the row it must occupy in that prompt's own output contract. `scripts/test-artifact-schema.sh` group `MG` reads this fence and **holds no copy of any row**: the anchor it searches for, the modes it compares, and the contract row it looks up all come from here, so a disagreement between this table and a prompt is a red check rather than a latent defect. A behaviour absent from this fence is ungraded — adding one is how it starts being graded, and the columns are `anchor | prompt | modes | contract-row`, pipe-separated, one row per branch.

```mode-gated-behaviour-register
# anchor | prompt | modes | contract-row
equity-aware disruption recovery | agents/05-hub-planner.md | ITERATION,RESEQUENCING | Recovery rebalance recorded
recovery-equity check | agents/06-validator.md | ITERATION,RESEQUENCING | Recovery equity (losses not concentrated; needs still hold)
full pass on all days after a resequence | agents/06-validator.md | RESEQUENCING | Full-pass day coverage after a resequence (all days, not only changed)
```

**What this register does not establish, stated so a green is not read as more than it is.** No arm behind it observes an agent *firing* any of these branches. Every one of them grades the **tree** — that the branch is declared, that this fence and the prompt agree on the modes gating it, that no clause exempts it, and that it is counted in the prompt's own output contract. Firing is **unassertable from a tree of prompt files**: a hand-built witness artifact passes identically whether an agent produced it or a person typed it, which is why no witness fixture is offered here. What would settle it is a live dispatch of the named agent against a trip at the branching mode, with the branch's own output observed in the artifact it writes.
...
**E2 is expected to be error-shaped when the trip population is zero** — the glob does not expand, so under `bash` `grep` is handed the pattern itself and errors, while under `zsh` the shell rejects the unmatched glob and `grep` never runs — so **when G2 resolves a zero population, E2 carries no information and is never read.** That declaration is about E2's **output**, and it stands unchanged. What follows is about its **exit status**, which is a separate property of the same entry and was conflated with the first here until the distinction was measured.

**Every entry terminates in an unconditional-success construct, and it is retained deliberately rather than carried over.** Under the retired `!` carrier it was load-bearing in the strongest sense: a non-zero exit in a pre-execution block is **fatal to the load**, aborting the consuming file before any instruction in it is reached, so a consumer whose block could fail was a consumer that could cease to exist. **Retiring the carrier removes that hazard entirely** — there is no pre-execution status left to abort anything, which is what makes every STOP below reachable as ordinary body text. What the construct still buys is stated in the next paragraph and is not the same thing. **E2's failing states are ordinary, not exceptional** — measured: with a zero trip population `grep` exits **2** under `bash`, while under `zsh` the shell rejects the unmatched glob and `grep` never runs (exit **1**); with a trip present whose `trip-context.md` matches none of the alternation arms above, it exits **1**. Both are states a healthy data home is in every day; neither is now capable of removing a verb.

The construct has two halves and each does a different job. `|| printf '<sentinel>\n'` makes the failure **visible**, because a `grep` that matches nothing exits non-zero while writing nothing at all to stderr: without the sentinel that state renders as an **empty result**, indistinguishable from a read that never ran. `; true` makes the status **unconditional** once the entry reaches it, so a tool result carrying a non-zero status can never be mistaken for a read that did not happen — the same distinction the sentinel draws, at the other end of the same entry. **Neither half substitutes for the other**, and the sentinels are chosen so they cannot be mistaken for data: `TRIPS-DIR-UNREADABLE` is not a trip directory name and is not `README.md`, so G1's canary cannot be spoofed by it and G2 never admits it to `trips[]`.

**One condition stops E2 before either half runs, and it lies in the shell rather than in this list.** Where `grep` is a shell function — as the harness's own shell snapshot defines it — an E2 glob that matches nothing abandons the whole entry: no sentinel prints, `; true` never runs, and the entry exits **1**. With an external `grep` the `||` branch runs as described above. The glob matches nothing in three states — a zero trip population; a `trips/` that is absent or unreadable; and no trip directory holding a `trip-context.md` the glob can reach — so in those states, where `grep` is a function, neither half runs and the entry's status is not unconditional. Measured on Claude Code 2.1.233 at `c79c434`, in the harness's own shell and again under `zsh -f` with `grep` defined as a function. **No gate misreads those states, and the reason is the ladder's order rather than a measurement:** at a zero population G2 stops on E1 and never reads E2; with `trips/` absent or unreadable G1 stops on E1's sentinel; and where no trip directory holds a `trip-context.md` the glob can reach, G3 needs the resolved trip's prefix in E2, the shell's error text does not carry it, and G3 stops with its message that the trip's `trip-context.md` is missing or unreadable.

**No entry is wrapped in a group, and the omission is load-bearing.** Each entry is a bare list — `<read> 2>&1 || printf '<sentinel>\n'; true` — and never `{ …; } ; true` or `( … ) ; true`. The shell already parses the bare list as the group did, so a wrapper adds no meaning: output and exit status are identical, measured over healthy, empty, unreadable and absent `trips/` directories and a trip whose context matches no arm, under both `bash` and `zsh`. What a wrapper adds is a refusal. Claude Code refuses a brace group that contains a quote character on its shape — under the consuming file's grants, and again with those grants widened to name `printf` and `true` — and refuses a parenthesised subshell with or without grants, while within the session's allowed working directories it admits the same list unwrapped. No grant buys the wrapper back, so the spelling, not the grant set, is what this list fixes. That refusal is the harness's behaviour, not the shell's — measured on Claude Code 2.1.233 at `b199dc1` — and it is meant to be re-derived against the harness in use rather than carried.

**`<data-root>` — what the entries resolve against, and where its value comes from.** Every entry above names `<data-root>`, and the agent substitutes one resolved absolute path for it before issuing the call. **The three operator stores are `<data-root>/trips/`, `<data-root>/people/` and `<data-root>/groups/`** — three children of one root, which is what they already are. **The only thing that changes here is how that root is named: no store moves, and nothing in this section relocates, copies or deletes operator data.** **The resolved root must also be a directory the session may read without asking, and for `/trip-publish` so must the pointer's own directory, `${HOME}/.travel-planner/`.** Claude Code refuses — in print mode — any evidence read of a path outside the session's allowed working directories, whatever the entry's spelling and whatever grants the consuming file holds; `/trip-publish` reads the pointer with `grep`, which the same boundary refuses. A directory outside the launch directory is admitted once it is passed with `--add-dir` or listed under `permissions.additionalDirectories` in a trusted settings layer — a workspace that has not been trusted ignores that entry in its project `settings.json`. Measured on Claude Code 2.1.233 at `b199dc1`.
...
- **Venue deduplication.** No venue appears as an anchor on one day and an alternative on another. Max 2 appearances total across the full itinerary. **The cap counts places, not entries** — two spellings of one venue are one venue, and the cap resolves against that venue's key rather than its display name (`reference/data-architecture.md` § *3.3 Venue — surrogate key, forced by measured evidence*). The hub builds venue-matrix.md to enforce this BEFORE writing the itinerary.
...
| `[ENRICH]` fields — `### Transit Access`, `### Walkable Proximity`, `## Weather Context`, `## Destination Baseline`, `## Events & Calendar` | the enrichment agent (`agents/00-enrichment.md`) | `[ENRICH]`-only, whichever command dispatched it. Contract: `agents/00-enrichment.md`. Layering: `reference/data-model.md` → *Who Writes What*. |
...
| a block not listed above | **no writer exists** | A new block gets an owner in this table before it gets content. |
...
└── trips/                    ← ships with README.md only; all trip content git-ignored
    ...
    └── <destination>-<year>/ ← one folder per trip
        ...
        ├── travelers/                 ← per-traveler trip files (gitignored), one .md per traveler — human-authored; an optional person: line points at that traveler's record in people/
        └── outputs/
            ...
            ├── cost-estimate.md       ← per-traveler cost projection (C21) — written by the hub, after event-status.md
