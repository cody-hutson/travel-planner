...
| **Destination ideation** | User is exploring where to go and no destination is chosen yet | Dispatch **Destination Ideation** to turn the group's leanings into a ranked shortlist (`outputs/destination-shortlist.md`) for the group to decide from. | "Where should we go?", "Give us some options for spring" | `/trip ideas` |
