...
| **Plan audit** | User wants the existing plan checked without changing it | Run the Validator alone against the current itinerary and report findings. No spokes, no hub, no edits. | "Does the plan hold up?", "Check the itinerary for problems" | `/trip check` |
