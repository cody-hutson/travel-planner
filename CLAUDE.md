...
| **Ambiguous — booked what** | User reports a booking and has not said whether the thing booked is a placed event | State the distinction and let the user pick: `fact` routes a trip-level booking to its block in `trip-context.md`, `event` changes a placed event's status on the itinerary and recomputes what that row derives. Propose the choice; never resolve it here. | "We booked the hotel", "We booked the Belem tour" | AMBIGUOUS: `/trip-record fact` · `/trip-record event` |
