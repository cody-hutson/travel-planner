...
E1  !`{ ls -1 "${CLAUDE_PROJECT_DIR}/trips" 2>&1 || printf 'TRIPS-DIR-UNREADABLE\n'; } ; true`
E2  !`{ grep -H -E '^\*\*Current mode:\*\*|^- \*\*Primary destination:\*\*|^\*\*Lifecycle:\*\*' "${CLAUDE_PROJECT_DIR}/trips"/*/trip-context.md 2>&1 || printf 'NO-TRIP-CONTEXT-READABLE\n'; } ; true`
```

`2>&1` is **mandatory** on every entry, for a stated reason rather than an assumed one. It is **not** what makes G1's canary work — the canary fires on an empty block too, because `README.md` is not a line of an empty block. It is what makes a STOP **diagnosable**, and what removes any dependence on whether `!` pre-execution captures stderr by default, which this repo has not established. Grants are `Bash(ls:*)` for E1 and `Bash(grep:*)` for E2 and nothing wider (`ADR-007` §2, bound 2); `/trip-new` carries the E1-only prefix and correctly stays at `Bash(ls:*)`.

**E2 is expected to be error-shaped when the trip population is zero** — the glob does not expand and `grep` errors — so **when G2 resolves a zero population, E2 carries no information and is never read.** That declaration is about E2's **output**, and it stands unchanged. What follows is about its **exit status**, which is a separate property of the same block, and the two were conflated here until the distinction was measured.

**Every entry terminates in an unconditional-success construct, and that is load-bearing rather than defensive.** A non-zero exit in a pre-execution block is **fatal to the load**: the block aborts the consuming file before any instruction in it is reached, so a consumer whose block can fail is a consumer that can cease to exist. The inner command still errors exactly as declared above; the construct keeps that error from taking the file down with it. **E2's failing states are ordinary, not exceptional** — measured: with a zero trip population `grep` exits **2**; with a trip present whose `trip-context.md` matches none of the alternation arms above, it exits **1**. A fresh clone is exactly the first of those, because `trips/README.md` is the only tracked file under `trips/` (`.gitignore`), so an unhardened E2 takes all four of its consumers down on every new install. E1's own failing state — an absent or unreadable `trips/` — is the second.

The construct has two halves and each does a different job. `|| printf '<sentinel>\n'` makes the failure **visible**, because a `grep` that matches nothing exits non-zero while writing nothing at all to stderr: without the sentinel that state renders as an **empty block**, indistinguishable from a block that never ran. `; true` makes the status **unconditional**, which is what keeps the load alive. **Neither half substitutes for the other**, and the sentinels are chosen so they cannot be mistaken for data: `TRIPS-DIR-UNREADABLE` is not a trip directory name and is not `README.md`, so G1's canary cannot be spoofed by it and G2 never admits it to `trips[]`.
