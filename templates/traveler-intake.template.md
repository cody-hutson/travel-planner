...
> **Anything the trip MUST accommodate for you?** These are the non-negotiables — the
> boundaries the plan has to stay inside. Each one is your personal stake in a trip-level
> rule (the heat ceiling, the mobility limit, the allergy) that lives in the trip's own
> file — so each need **points at** that rule via "Applies to" rather than restating it.
> Fill in *your* specific detail; leave "Applies to" blank if you're not sure which rule
> it falls under, and an agent will link it for you.
>
> **Add a block for each need you have, and delete the rest.** If you have a real need
> the trip hasn't captured as a rule yet (say, a rest floor), still write it here — an
> agent flags it so it can be added. Pick the closest **Category** for each:
...
- **Applies to:** [e.g., `Hard Constraints → "Limited stair tolerance"`. Leave blank if unsure.]
...
- **Applies to:** [e.g., `Dietary & Health → "Allergies"`. Leave blank if unsure.]

> Add as many need blocks as you like; delete the spares. Each need is exactly one
> Category. A need is non-negotiable — if it's more of a "would be nice," it belongs
> under Desires below.
