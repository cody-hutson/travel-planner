...
surface for status; `trips/<slug>/outputs/change-summary.md` — its dated entries
...
**Neither the file's presence nor its `status` field is the signal, and the second of those is
a correction.** Presence was rejected first and for a reason that still holds: C20 is
`accumulate-append`, so the file exists from the first summary onward and outlives every
...
every open of what the build produced, and the prune can only remove a band that was already
dead. Without it a trip whose one change was confirmed a year ago carries an `updated`
frontmatter, a band node, a CSS rule and a script branch in every render for the rest of its
life; AC 5 asks that a site with no pending change and no *recent* update render exactly as it
