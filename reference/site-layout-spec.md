...
`site` verb's read scope in `skills/trip/SKILL.md`. This section owns what the render *does* with the state and the
...
**Four classes carry `internal-hard`, and only two of them are § 9.3 exclusions.** That is not an
...
drops. **`people/<person>.md` and `groups/<group>.md` are not per-trip artifacts at all**: they are
the durable person record and the reusable-group record, held at the repo root outside every trip,
and no build input set ever reaches either. Listing them in 9.3 would assert that the build
considered and declined them, which is the opposite of true. **They are here because group `PB`
compares this fence against § 1.1's publish column in both directions** — a class carrying a declared
publishability and no fence row is a failure whether or not the site build can see it, and that is
what keeps this the single home of the assignment rather than a list of what the renderer happens to
touch. **The cross-trip half of that set grows with the model rather than staying at one**, which is
why this paragraph names the property those rows share instead of naming the one class that had it.
