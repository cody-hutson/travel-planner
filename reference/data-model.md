...
**The trip-day set** is the trip's own days: the days published by `trip-context.md` `## Logistics` → `### Effective Planning Days [DERIVED]`, from its `Arrival day` line through its `Departure day` line inclusive, taken as published. That block is the one home for the trip's day span, exactly as the per-traveler block beside it is the one home for a window. **A trip day is a member of that set** — *trip day* and *trip-day set* are the singular and the set form of one term, and no reading turns on which spelling a sentence uses.

**That block answers which days the trip has; it never answers who is present on one.** Its own scope note declines to be read as a guarantee that every traveler is present for all of it, and routes every presence question to each traveler's own derived window — which is this predicate's first limb below. The two compose rather than compete: the trip-day set is what the predicate ranges over, the per-traveler window is what decides membership on a day of it, and neither is read for the other's question.

**A window reaching outside the trip-day set does not extend it.** Where a traveler's own effective window reaches a date the trip's days do not contain — an early arrival, a late departure — that date is **not** a trip day and is **not** in that traveler's present-day set. The traveler is at the destination and the trip is not planning there: no day shape exists to place into and no plan-day exists to honor or to breach, which is the same reading that makes **absent** the presence failure that removes a grade. Taking the trip's days as the union of the travelers' windows instead would let one traveler's window add a day to the trip, and would draw the trip's day population from the block that moves whenever who is present moves.
...
A **field** is a distinct `(section, label)` pair rendered as a body bullet of the form `- [⭐ ]**<Label>:**` **above** the `<!-- PROFILE-END -->` sentinel of **either** guided intake form — `templates/traveler-intake.template.md` (the trip form) or `templates/person-intake.template.md` (the durable person form) — **or** inside a region of the block-owned `templates/trip-context.template.md` that `reference/adr/ADR-024-form-contract-writer-boundary.md`'s disposition table predicts interviewable, **YES** or **CONDITIONAL**. **The sentinel is a whole line:** the one line consisting of exactly `<!-- PROFILE-END -->`, trailing whitespace aside. Each guided form's `intake-form` fence names the same literal on its own `boundary:` line, and that line is never the sentinel. **All three, because the question set is the three forms together and any one of them alone is part of it.** Their pair sets are **disjoint** — the two guided forms share no label, and no trip-context section is a section of either — so their union is a plain sum with nothing double-counted. Repeated blocks (`Needs` ×3, `Desires` ×3; trip-context's `### [Constraint Name]` blocks, its journey legs and its `#### Origin <letter>` units) are **one** field each — the forms' own instructions are *"add as many as you like and delete the rest"*, *"Add or remove blocks as needed"* and *"Add one `#### Origin <letter>` block per additional origin"*, so block count is user data, not schema. **A label inside such a repeated block is `block`-scoped; every other label is `slot`-scoped.** That is the `Scope` column below and the axis `K3`/`K4` and `MALFORMED-SLOT` branch on — stated here as a rule rather than left to be inferred from this paragraph two sections later.

**A bullet's section is the `Section` cell that its enclosing `## ` heading begins with**, provided the rest of the heading is empty, or opens (after optional whitespace) on a character that is neither a letter, a digit nor whitespace — so `Needs` leads `Needs — the must-haves`, and `Destination` does not lead `Destination leanings`.
...
| trip-context fields in a region `ADR-024` predicts **NO** — the five `[ENRICH]` blocks, the two `[DERIVED]` blocks, and `## Mode` | **No row, deliberately.** A row would assert an interviewability that record refuses. `## Mode` carries its fields as bold lines rather than bullets, so the key cannot see them whoever writes them (`ADR-024` R2) |
| trip-context's unlabelled regions — the title line, the `## Group` roster table, `## Soft Preferences`, `## Trip Style`, `## Possible Day Trips`, `## Locked Elements`, `## Validation Requirements` and `## Notes for All Agents` | **Not classified in this revision — declared rather than silent.** Each is answerable and carries no `(section, label)` pair, so the key the form contract joins on cannot address it. The guided forms' free-text tail is classified because composition could otherwise carry trip-specific text into a durable record; no trip-context region is composed with any record, so a class here would have no consumer |

**The denominator is stated as a rule, not as a number.** A literal count in prose goes stale the first time a form gains or drops a line; the rule above does not — **provided it names every form that renders a field**, which is the failure mode the split demonstrated and the trip-context backfill met again, rather than a hypothetical one. Applied to the three shipped forms the rule measures **76 distinct labelled bullets — 19 on the trip form, 17 on the person form and 40 in trip-context's interviewable regions, disjoint.** Trip-context's 40 are the same whether its bullets are read line-initially (50 bullets) or at any indent (56, the instrument `ADR-024` counts with), because every indented bullet there repeats a pair already rendered line-initially. The classification table below carries one row per answerable slot, and it differs from the bullet count by exactly the two labels that are fields but not bullets: **`Name`**, borne by the H1 title line, and the `Needs` block's **`Applies to`**, never asked because it is computed. **19 + 17 + 40 = 76 bullets, + `Name` + `Applies to` = 78 labelled fields, + 1 unlabelled free-text slot = 79 answerable slots.** That last figure is the table's **declared row count**: `scripts/test-artifact-schema.sh` group `FT` reads it from this sentence and grades it against the rows the table actually carries, so a row added or removed without this sentence moving with it turns the gate red. The same group grades the converse direction too: every labelled row must be reached by a bullet of a form this rule names, unless its own rationale declares it bullet-less, so a row that outlives its field turns the gate red as well. Every number here is reproducible from the three forms and the table rather than asserted, and the reconciliation is written out so that a future form change **moves a countable that can be re-derived** instead of silently invalidating one that cannot.
...
`Scope` = how many times the label may legitimately occur in one file. **`slot`** = at most once, so a second occurrence is `MALFORMED-SLOT`. **`block`** = the label belongs to a repeated block (`Needs`, `Desires`; trip-context's constraint blocks, journey legs and `#### Origin` units), so repetition is **user data, not a defect** — the same reading the denominator rule above already applies when it counts a repeated block as one field. **Scope is a property of the field, total over every row of the table below, and it is the axis `K3`/`K4` and `MALFORMED-SLOT` branch on.**

`Horizon` = does this field **declare** a validity horizon? **`required`** = a `[VALID-THROUGH H]` mark is expected on the record's value, and its absence or unparseability is a defect, reported as `HORIZON-UNCONFIRMED`. **`admissible`** = a mark is honoured where it is present and its absence is not a finding — the disposition every field carries by default, and the one `reference/schemas/person-record.md` already describes when it calls the mark *admissible on any field bullet*. **A mark that is present but malformed is a finding on any field. `admissible` governs whether a mark is owed, never whether a present one may be ignored.** **Horizon is a property of the field, total over every row of the table below and single-valued, exactly as `Class` and `Scope` are.** It is what makes the horizon *declared* rather than merely *permitted*: an instance may always carry a mark, and this column is where a field says one is owed.

> **The generality is a procedure, not a promise.** To admit a second horizon-bearing field, set that field's `Horizon` cell to `required`, and amend `HZ2`'s declared membership in `scripts/test-artifact-schema.sh` in the same commit. **No rule changes, no extractor changes, no agent prompt changes, and no fixture outside that member's own** — one table cell and one guard **expectation**, and the guard's re-read is the point: an arm that accepted any membership would let a field acquire an obligation nobody declared. **The generality is that membership is data — nothing branches on the field's name** — not that the change is invisible. A `Passport`-shaped implementation could not make that claim; a field-level axis can, which is what keeps the mechanism from collapsing into a special case for one field.

<!-- Column position is load-bearing, not cosmetic. `scripts/test-artifact-schema.sh` parses
     the rows below with `awk -F'|'` BY INDEX in one extractor, `ft_rows`, which every group
     that reads this table goes through; the two EXTRACT controls only strip rows by `$2`.
     `ft_rows` reads `$2` through `$9` — `#`, `Field`, `Section`, `Class`, `Ovr?`, `Scope`,
     `Horizon`, `Rationale`. A column inserted anywhere shifts `Class` or `Scope` for every
     one of those groups at once, and they would then grade a class-blind table WHILE THE
     SUITE REPORTS GREEN; group `FT`'s vocabulary arm is what turns that red.
...
| 32 | `Been here before?` | Interests & tastes | **DEST** | n/a | `slot` | `admissible` | *"How well you already know **this destination**"* — closed enum; unanswered reads **unknown**, never `never`. The trip-history hook is **realised** by `/trip-record history`, which offers `once` or `a few times` from an operator-confirmed prior-visit count and refuses `know it well` (a familiarity claim a count cannot ground) and `never` (absence of a reference is not absence of travel) |
| 33 | `Already done` | Interests & tastes | **DEST** | n/a | `slot` | `admissible` | *"Anything you've already seen or eaten **here**"*. The trip-history capability is **realised** by `/trip-record history`, and it *supplements* these two rather than replacing them: it offers a pointer to the prior trips on file, authors no prose, and writes neither field |
...
| 40 | `Primary destination` | Destination | **TRIP** | n/a | `slot` | `admissible` | The destination **this trip** goes to — the subject is the trip's choice (T1). Not `DEST`, which is a fact *about* a destination that no choice of trip changes; this is the choice itself |
| 41 | `Secondary destinations` | Destination | **TRIP** | n/a | `slot` | `admissible` | This trip's day trips and multi-city legs; another trip to the same primary destination may take none |
| 42 | `Neighborhood base` | Destination | **TRIP** | n/a | `slot` | `admissible` | Where this group bases itself **this time**; another trip to the same city may base elsewhere, which is why it is not `DEST` |
| 43 | `Primary traveler` | Logistics | **TRIP** | n/a | `slot` | `admissible` | Names whose journey the anchor legs (`Origin A`) describe, and a trip has exactly one anchor. The template says it is not a seniority label. The traveller's own durable `Leaving from` stays `DEFAULT` on their record; this line only attaches this trip's anchor origin to a roster name |
| 44 | `Confirmation code(s)` | Logistics | **TRIP** | n/a | `block` | `admissible` | This trip's booking codes. **One pair, two meanings:** the trip-level line and the line inside an `#### Origin` block that books separately. The key cannot tell them apart and does not need to — both take the same value on every axis here. `block` because the per-origin copy repeats with its unit |
| 45 | `Leg 1` | Logistics | **TRIP** | n/a | `block` | `admissible` | A flight leg of this trip. Repeats under `### Outbound`, `### Return` and every `#### Origin` block; the `##`-level key folds every journey's legs into this one row, which loses nothing because they share every axis value |
| 46 | `Leg 2` | Logistics | **TRIP** | n/a | `block` | `admissible` | As `Leg 1` |
| 47 | `Notes` | Logistics | **TRIP** | n/a | `block` | `admissible` | Per-journey notes — layover, bags, seat class — repeating with each journey, as `Leg 1` |
| 48 | `Departing travelers` | Logistics | **TRIP** | n/a | `block` | `admissible` | Which roster members leave from an additional origin on this trip. **`block` although the template renders it once:** it belongs to the `#### Origin <letter>` unit the form tells its filler to repeat per additional origin, so a trip with two extra origins carries it twice, legitimately |
| 49 | `Property name` | Accommodation | **TRIP** | n/a | `slot` | `admissible` | The property booked or considered for this trip. The durable lean it answers is the person form's `Lodging style` (`DEFAULT`); this is the choice that lean informed, not a copy of it |
| 50 | `Booking status` | Accommodation | **TRIP** | n/a | `slot` | `admissible` | The state of this trip's lodging booking. Not the per-event status model, which lives in `outputs/event-status.md` |
| 51 | `Address` | Accommodation | **TRIP** | n/a | `slot` | `admissible` | The address of this trip's property |
| 52 | `Room type` | Accommodation | **TRIP** | n/a | `slot` | `admissible` | The room booked for this group's size and bed configuration |
| 53 | `Key amenities` | Accommodation | **TRIP** | n/a | `slot` | `admissible` | Amenities of this trip's property that change the plan |
| 54 | `Check-in time` | Accommodation | **TRIP** | n/a | `slot` | `admissible` | This booking's check-in time, which bounds the first day |
| 55 | `Check-out time` | Accommodation | **TRIP** | n/a | `slot` | `admissible` | This booking's check-out time, which bounds the last day |
| 56 | `Total travelers` | Group | **TRIP** | n/a | `slot` | `admissible` | The size of this trip's party. **Its writer is conditional** — one command at creation, another thereafter — and that is a writer-partition fact the contract's `(region × condition)` key carries (`ADR-024` decision 1); the classification is unconditional |
| 57 | `Travel mode` | Group | **TRIP** | n/a | `slot` | `admissible` | How this group moves on this trip — together, or in subgroups. Same conditional writer as `Total travelers` |
| 58 | `Subgroup notes` | Group | **TRIP** | n/a | `slot` | `admissible` | Which members split off, on which days of this trip. Same conditional writer as `Total travelers` |
| 59 | `Description` | Hard Constraints | **TRIP** | n/a | `block` | `admissible` | What this trip's constraint is. The constraint block repeats — *"Add or remove blocks as needed"* — so every label in it is `block`. The trip-level text is the source a traveller's need **links** to, never a copy of it: the durable need itself stays `Specific` (`PERSON`) on the record |
| 60 | `Applies to` | Hard Constraints | **TRIP** | n/a | `block` | `admissible` | **The constraint → people edge**, the inverse of the need → constraint edge `(Needs, Applies to)` that shares its label (§ *A need that applies to one trip only*). It names this trip's group members, so `TRIP`. **This row is why the join key is `(section, label)`:** keyed on the label alone, a bullet here resolves to `(Needs, Applies to)` |
| 61 | `Practical impact` | Hard Constraints | **TRIP** | n/a | `block` | `admissible` | What the constraint means for this trip's scheduling and activity selection |
| 62 | `Time blocks affected` | Hard Constraints | **TRIP** | n/a | `block` | `admissible` | Which of this trip's time blocks the constraint binds |
| 63 | `Mitigation approach` | Hard Constraints | **TRIP** | n/a | `block` | `admissible` | How this trip accommodates the constraint |
| 64 | `Bailout requirement` | Hard Constraints | **TRIP** | n/a | `block` | `admissible` | Whether this trip's long outdoor blocks need a planned indoor escape for this constraint |
| 65 | `Overall tier` | Budget Posture | **TRIP** | n/a | `slot` | `admissible` | This group's spend tier for this trip. The per-person durable leans are `Comfort range` and `Splurge appetite` (`DEFAULT`); this is the group setting they inform, not a copy of either |
| 66 | `Meals` | Budget Posture | **TRIP** | n/a | `slot` | `admissible` | This group's meal budget for this trip, as `Overall tier` |
| 67 | `Experiences` | Budget Posture | **TRIP** | n/a | `slot` | `admissible` | This group's willingness to pay for premium experiences on this trip |
| 68 | `Low-stakes acceptable` | Budget Posture | **TRIP** | n/a | `slot` | `admissible` | Whether this group takes convenience-store, food-court or stall meals on this trip |
| 69 | `Spend priorities` | Budget Posture | **TRIP** | n/a | `slot` | `admissible` | Where this group spends and where it stays lean on this trip |
| 70 | `Allergies` | Dietary & Health | **TRIP** | n/a | `slot` | `admissible` | **The trip-level constraint source** for this group's allergies. A traveller's need links here through its `Applies to`; the durable allergy itself is that traveller's `Specific` (`PERSON`). Not `PERSON` here, because this line is the group's constraint for this trip and has no durable value beneath it |
| 71 | `Dietary restrictions` | Dietary & Health | **TRIP** | n/a | `slot` | `admissible` | As `Allergies`, for religious, ethical and medical restrictions |
| 72 | `Dietary preferences` | Dietary & Health | **TRIP** | n/a | `slot` | `admissible` | This group's eating preferences for this trip. The per-person `Cuisine appetite` (`DEFAULT`) informs it; this is not a copy of it |
| 73 | `Mobility notes` | Dietary & Health | **TRIP** | n/a | `slot` | `admissible` | As `Allergies`, for the mobility limits that bound this trip's walking, stairs and terrain |
| 74 | `Other health notes` | Dietary & Health | **TRIP** | n/a | `slot` | `admissible` | As `Allergies`, for anything else that shapes this trip's pacing or environment |
| 75 | `Itinerary file` | Current Itinerary Status | **TRIP** | n/a | `slot` | `admissible` | Where this trip's current itinerary lives |
| 76 | `Current version` | Current Itinerary Status | **TRIP** | n/a | `slot` | `admissible` | The version of this trip's itinerary |
| 77 | `Locked elements in current plan` | Current Itinerary Status | **TRIP** | n/a | `slot` | `admissible` | The operator's summary of what this plan may not change. `outputs/event-status.md` stays the structured source of truth; this is the human summary the write-ownership table gives the operator |
| 78 | `Open for change` | Current Itinerary Status | **TRIP** | n/a | `slot` | `admissible` | What this trip's plan may still change |
| 79 | `Specific change requested` | Current Itinerary Status | **TRIP** | n/a | `slot` | `admissible` | The change being requested of this trip's plan |

**Totals — `PERSON` 3 · `DEFAULT` 15 · `TRIP` 58 · `DEST` 2 across the labelled fields**, plus the free-text slot as `TRIP`. **Scope totals — `slot` 60 · `block` 19**, the nineteen being the three `Needs` and five `Desires` labels and trip-context's eleven repeated-unit labels: its six constraint labels, `Leg 1`, `Leg 2` and `Notes`, and the `#### Origin` unit's `Confirmation code(s)` and `Departing travelers`. **Horizon totals — `required` 1 · `admissible` 78, and 1 + 78 = 79**, reconciling against the same row count the denominator rule above derives; `Passport` is the sole `required` member. **No trip-context row is `PERSON`, `DEFAULT` or `DEST`:** T1 decides every one — each field's subject is this trip's party, bookings or choices, and trip-context is composed with no durable record, so none has a durable value beneath it. Every slot carries exactly one class, **exactly one scope and exactly one horizon**; none carries two of any, and none is unclassified. **Sum each axis's column and assert equality with the row count the denominator rule above reconciles — never with a literal restated here.** Group `FT` asserts both halves: every row's cell on each axis inside that axis's closed vocabulary, and the row count equal to the one the denominator rule declares.
...
| **DEST** | the trip's `travelers/<traveler>.md` | the person record — the trip-history capability is **realised** by `/trip-record history`, and it supplies a *suggestion*, never an authority: it offers only where the field is unanswered and writes neither field, so what lands in the trip file is always the traveller's own answer | **Trip wins.** Direct entry stays available and stays authoritative for the trip | `UNKNOWN`, never an empty set |
...
> **`Name` appears in neither column, and the split is why.** It was starred while the intake was one form; the person form now carries the name as its **H1 title line** rather than as a bullet, so it is starred nowhere and the star total is 9 rather than 10. It remains `DEFAULT` and remains library-sourced — a returning traveller is **not** re-asked it — but it is not part of the *starred* pass, which is what makes that pass 9 → 3 rather than 10 → 3.

**What holds these figures, and what it does not reach.** `scripts/test-artifact-schema.sh` group `ST` grades two different propositions about the starred set, and the difference between them is the whole of what a reader can rely on here.

- **Inside each form.** The group checks that the form's banner numeral agrees with the fields the form actually marks, and that the banner is the only sentence in the form stating the count **in the shape the group reads**: a numeral followed, within three intervening words, by *field*, *fields* or *of them*, on a line that names the star. A restatement worded any other way is not read at all, so this half pins one statement of the count in that shape rather than in every wording of it. That is agreement, not accuracy — a banner and a marked set that moved together onto a wrong number agree perfectly, and this half stays green on exactly that edit.
- **Against this table.** The group also grades the **level**. Per form, the labelled bullets that form carries, intersected with the starred rows of the field table above, must equal the set of fields that form marks; and across every form in one run, the union of those marked sets must equal this table's starred set. Both comparisons run in both directions, field for field, each bullet resolved on its `(section, label)` pair. The second is what catches a star added to this table for a field no form asks — the one input no per-form comparison can see.

So the **star column of the field table above** is **held by an assertion** rather than by a convention, over **every row of that table that carries a code-span field label**: a star that appears there and on no form, or on a form and not there, turns the gate red in the same run. **One row is outside that population, and it is outside by construction rather than by omission** — the unlabelled free-text tail row carries no code-span label, so it names no field, has no bullet on any intake form, and there is nothing on either side of the comparison for it to be matched against. The extractor admits a row on the presence of that label, so the exclusion follows from what the row is and needs no list to maintain: any future row without a field label is outside for the same reason, and any row that gains one is inside without an edit here. **The figures in this section are not themselves graded.** The totals in the prose above, and the two-column table restating them, are *readings* of that column rather than inputs to the comparison: they sit outside the population `ST` grades, and an edit that moves a numeral here and leaves the star column alone turns nothing red. Rely on the star column; treat the prose as a restatement that a reader should re-derive rather than trust. **The gate spells no number.** It is keyed on which fields carry a bullet, so these figures are a reading of this table and the shipped forms rather than a copy somebody has to remember to update — and a further intake form extends the check without editing it.

**What the gate does not reach.** Nothing in the shipping path derives, consumes or acts on how many fields carry the star, so there is no producer to grade these figures against and the group does not claim one. What it offers instead is a second maintained document agreeing with the forms field for field: stronger than a form agreeing with itself, weaker than a count checked against the code that computes it. An acceptance criterion written against this gate should buy the first and not the second.
...
**The record side is three-valued, because a horizon is a third state:** `UNSTATED` · `ANSWERED` · `EXPIRED`. **The third state is decided against a reference month `R`, not against the clock alone.** Zero-padded `YYYY-MM` compares lexicographically **and** chronologically, so `>` stays a string comparison with no date library — which is what keeps the predicate available on a pure-bash guard surface. § *The reference month* below is normative for `R`, and for the trip term `T` and wrapped term `W` it defines; § *Field Scope → The classification* is normative for the `Horizon` axis.

**`ANSWERED()` is evaluated first; the horizon question is asked only of a value that exists.** An em-dashed, blank, absent or still-bracketed `Passport` is `UNSTATED` — there is no value to bear a horizon, and `UNSTATED` is already that condition's state. The third state reaches a field only where `ANSWERED()` is true, which is why a domestic traveller told to *"Skip for a domestic trip"* is never asked to confirm a horizon on a document they do not carry.

A mark's payload is **well-formed** iff it matches `^[0-9]{4}-(0[1-9]|1[0-2])$`. That admits `2026-09` and rejects `2026-13`, `2026-00` and `2026-1` — each of which would break the lexicographic-equals-chronological property the whole comparison rests on, so admitting one would silently withdraw the guarantee rather than merely tolerate a typo. **A value composes the third state when any of the following holds:**

- **(i)** it carries a mark whose payload is well-formed and **not** `H > R`;
- **(ii)** it carries a `[VALID-THROUGH …]` mark whose payload is **not** well-formed — **on any field, whatever its `Horizon` axis**;
- **(iii)** it sits on a field whose `Horizon` axis is `required` and carries no mark;
- **(iv)** the comparison against `R` would otherwise **pass**, the trip term `T` did not resolve, and the field's `Horizon` axis is `required`;
- **(v)** the comparison against `R` would otherwise **pass** — `H > R` — the wrapped term `W` is defined, `H` is at or below `W`, and the field's `Horizon` axis is `required`.

**Clause (ii) is axis-independent deliberately:** `admissible` governs whether a mark is **owed**, never whether a present one may be ignored, so a mark that is present but malformed is a finding on any field. **Clauses (iv) and (v) are disjoint by construction** — (iv) presupposes that `T` did not resolve, while (v) presupposes `W`, which is defined only from a title year and month that did — so no value is ever reported under both. **(v) sits downstream of (ii)**, requiring a well-formed payload, and both live inside the `ANSWERED()`-true branch; neither can weaken the malformed-mark limb or the precedence sentence above. **Each of (iv) and (v) only ever moves a value from `ANSWERED` to the third state**, so both are monotone in the direction the outer `max` already is.
...
**Resolving `T` — one governed source, refined upward, and no search.** The term is taken from the trip's own title line and then refined by at most one further reading of the same file. Nothing here scans a directory, walks a tree, or consults a second file; the resolution terminates whether or not the refinement finds anything, and it is single-valued.
...
> 3. **No such `(M_title, Y_title)` pair on the title line → `T` does not resolve.** The non-resolving condition is the pair's absence, not the year's: a title line carrying a bare year that no month name precedes yields no term either, because step 1 never selects it. `R` is the clock. A value the clock alone would already have expired is still `EXPIRED` — that verdict needs no trip term. A value the clock would have **passed** composes the third state and is reported `HORIZON-UNCONFIRMED` where the field's `Horizon` axis is `required`: *a horizon that cannot be compared against the trip has not been shown to cover it.* **The degraded case prompts; it never passes silently.**

**The inner `max` is the outer one, one level down.** It is the same operator for the same reason: it can only move the term later, so it can only make `R` larger, and it therefore adds no failure mode the outer `max` does not already carry. A first-match ladder has the opposite property — a wrong first step is decisive — which is why the term is an expression over both months rather than an ordered fall-through.

**`Departure day` is the trip's LAST day, not its first.** In § *Effective Planning Days [DERIVED]* the bullets run `Arrival day` → `Full planning days` → `Departure day` in ascending date order, and the template's own gloss reads *"Departure day — must depart hotel by [time] for [departure time] flight."* That is what makes it the window-end proxy, and it is why `M_dep` earlier in the year than `M_title` reads as a **year boundary** rather than as an early start. A reader who assumes the opposite inverts the whole of the next subsection.

**The precision matches what the form collects.** The intake asks for *"the month it's valid through — never the number"*, and the title line carries a month and a year as a unit. Composing a `YYYY-MM` out of a day-precision source that carries no year would manufacture precision the form declined to ask for, which § `ANSWERED()` already forbids.

#### The New-Year case — the wrapped term, and the window where the two readings disagree

A trip titled by a December month and departing in January resolves `T = Y_title-12`, where the trip in fact ends in `(Y_title + 1)-01`: the resolved term is **early, in the permissive direction**. At month precision that same observation — `M_dep` earlier in the year than `M_title` — is equally produced by a **stale title**, and the two cannot be told apart. Reading every such trip as a wrap would over-strictify by up to a year on what is usually a defect state; reading none of them as a wrap leaves a genuine year-spanner uncovered. **The rule fires only where the two readings actually disagree about the verdict.**

> **`W` — the wrapped term.** Defined **only** where `Y_title`, `M_title` and `M_dep` all resolve **and** `M_dep` is earlier in the year than `M_title`, as **`W = (Y_title + 1)-M_dep`**. It is the term the New-Year reading would have produced. Where any of the three does not resolve, or where `M_dep` is not earlier than `M_title`, **`W` is undefined and the clause below is silent.**
>
> **The disputed window is the half-open interval `(R, W]`** — exactly the horizons on which the resolved reading is silent and the wrapped reading prompts. A value whose horizon lands in it composes the third state and is reported `HORIZON-UNCONFIRMED`; the remedy is to **restate the title line so its month and year name the month the trip ends in**, which clears every such prompt on that trip at once.

**`W` is a function of the title and departure months alone — never of the clock.** The clock enters only through the *would otherwise pass* limb, and that is the whole of the clipping story:

| Clock position | `R` | Disputed window `(R, W]` | Behaviour |
|---|---|---|---|
| clock at or before the trip term | the trip term | `(T, W]` — the full disagreement interval | the clause fires across the whole interval |
| clock past the term but not past `W` | the clock | `(clock, W]` — **clipped from below** | horizons at or below the clock are already third-state on the clock's own account; the *would otherwise pass* limb is what stops the clause re-labelling them with the wrong remedy |
| clock at or past `W` | the clock | **empty** | the clause is **unreachable** — on a past-dated or overtaken trip the New-Year question is moot, and the clock guarantee already covers everything the wrapped reading would have asked for |

That last row is the outer `max`'s own argument, one level down again: the wrapped reading can only ever *add* a prompt, never withdraw one.

**Stated residuals.** Where the term resolves but the trip carries no derived departure month, `W` is undefined and the value passes on the resolved term — a year-spanning trip with no `[DERIVED]` block is not detectable at month precision, and prompting there would be the benign-staleness prompt this rule exists to avoid. And the window widens with the gap between the two months, so a title month far later than the departure month — most plausibly a stale title rather than a long year-spanner — prompts across a correspondingly wider range. That cost is accepted deliberately: the prompt is never a false positive **about the record**, because a title month later than the derived departure month is a self-inconsistent trip file either way, and one edit to the title line clears it.

**The verdict cases, declared here and graded from here.** The block below is the oracle for the rule above, in the same declaration-in-the-document form as § 10's freeze declaration and count-assertion fence in `reference/data-architecture.md`: `scripts/test-artifact-schema.sh` group `HZ` reads it live and holds no copy of it. **`verdict-clock` is a control column, not a record of what shipped previously**: it is *this* predicate with `R` forced to the clock month and the trip-term clauses — the unresolvable-term clause and the wrapped-term clause — not in force. The two columns therefore differ in exactly one variable, the reference month, which is what makes a difference between them attributable. A `wrap-term` of `—` means `W` is undefined for that case; an `H` of `—` means the value carries no mark at all.

```horizon-verdict-cases
# id   clock    trip-term  wrap-term  H          axis        verdict-clock        verdict-trip
C1     2026-09  2027-04    —          2026-12    required    ANSWERED             EXPIRED
C2     2026-09  2027-04    —          2027-04    required    ANSWERED             HORIZON-UNCONFIRMED
C3     2026-09  2027-04    —          2027-05    required    ANSWERED             ANSWERED
C4     2026-09  2027-04    —          2026-08    required    EXPIRED              EXPIRED
C5     2026-09  2026-04    —          2026-12    required    ANSWERED             ANSWERED
C6     2026-09  2026-09    —          2026-09    required    HORIZON-UNCONFIRMED  HORIZON-UNCONFIRMED
C7     2026-09  2027-04    —          2026-12    admissible  ANSWERED             EXPIRED
C8     2026-09  2027-04    —          later      admissible  HORIZON-UNCONFIRMED  HORIZON-UNCONFIRMED
C9     2026-09  2027-04    —          2030       admissible  HORIZON-UNCONFIRMED  HORIZON-UNCONFIRMED
C10    2026-09  2027-04    —          2026-13    required    HORIZON-UNCONFIRMED  HORIZON-UNCONFIRMED
C11    2026-09  —          —          2027-05    required    ANSWERED             HORIZON-UNCONFIRMED
C12    2026-09  —          —          2027-05    admissible  ANSWERED             ANSWERED
C13    2026-09  2027-04    —          —          required    HORIZON-UNCONFIRMED  HORIZON-UNCONFIRMED
C14    2026-09  2027-04    —          —          admissible  ANSWERED             ANSWERED
C15    2026-09  2026-12    2027-01    2027-01    required    ANSWERED             HORIZON-UNCONFIRMED
C16    2026-09  2026-12    2027-01    2027-02    required    ANSWERED             ANSWERED
C17    2026-09  2026-12    2027-01    2026-12    required    ANSWERED             HORIZON-UNCONFIRMED
C18    2026-09  2026-12    2027-01    2027-01    admissible  ANSWERED             ANSWERED
C19    2027-06  2026-12    2027-01    2027-08    required    ANSWERED             ANSWERED
C20    2027-01  2026-12    2027-02    2027-02    required    ANSWERED             HORIZON-UNCONFIRMED
C21    2026-09  2026-12    —          2027-01    required    ANSWERED             ANSWERED
```
...
**Store-root resolution: `<trip-root>/people/` when that directory exists, otherwise `<data-root>/people/`. Two steps, deterministic, terminating; no upward search.** Step 1 is unchanged and needs nothing new: `<trip-root>` is the resolution contract's own `trip.path`, which is `<data-root>/trips/<slug>`, so a per-trip store resolves off the same root transitively. Step 2 names the **data root** — the directory the operator's three stores are children of, resolved by `CLAUDE.md` § *Resolving a trip* gate `G0-root` — and **never the engine root**.

The rule is forced rather than chosen on style, and its former justification was measurably wrong. It used to read: *`.gitignore` carries `/people/*` rooted, so the repo-root store does not exist in a fresh checkout.* Measured: `people/README.md` is **tracked** and re-included by `!/people/README.md`, so the directory **does** exist in a fresh checkout, holding exactly one file. The correct reason is stronger than the one it replaces, and it is the whole argument for naming the data root here:

> The engine ships an **empty store skeleton** — one tracked `README.md` per store — which satisfies every presence and readability predicate a fallback can test while holding zero records. A step-2 fallback rooted at the engine therefore returns `RESOLVED` on the directory and `DANGLING` on every record, and for the publish guard it returns `UNDETERMINED`, which aborts the publish. **A witness whose verdict depends on which copy of the engine is on the path is not a witness** — and the original concern survives inside this one: a verdict that differs between an author's machine and CI is the same defect seen from one angle.

An upward search is rejected for a second reason, unchanged: it is non-deterministic when both roots exist, which reintroduces exactly the order-dependence the next-but-one subsection forbids.
...
| `HORIZON-UNCONFIRMED` | the horizon's own month **is** the reference month, so month precision cannot settle whether it covers the trip; **or** a **value** carries a `[VALID-THROUGH …]` mark whose payload is not a well-formed `YYYY-MM`, on any field whatever its axis; **or** a **value** on a field whose `Horizon` axis is `required` carries no mark; **or** the trip's reference month did not resolve, the field's `Horizon` axis is `required`, and the comparison against the clock alone would otherwise pass; **or** the value's horizon lies at or below the term the New-Year reading would have produced, the comparison against the reference month would otherwise pass, and the field's `Horizon` axis is `required` | yes — remedy, selected by which condition fired: *state the horizon on the record* for the first three; *state the trip's month and year on the title line of `trip-context.md`* for the fourth; *restate the title line of `trip-context.md` so its month and year name the month the trip ends in* for the fifth |
