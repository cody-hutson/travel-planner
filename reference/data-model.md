...
**The joins already exist.** The `Venue` column carries the venue's `ven-<token>` and is *a reference, not this table's key* (below) — it resolves to that venue's researched entry, which carries its `Price range`. The desire an event serves resolves through `outputs/traveler-model.md`, the derived per-traveler projection the hub and the validator already read. This section adds no key and asks for none.
