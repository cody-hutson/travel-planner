...
witness: examples/data-architecture-demo/outputs/satisfaction-metrics.md
...
- **Coverage.** This class declares `witness:`. `examples/data-architecture-demo/` supplies the first tracked instance this class has ever had, and it is the only witness in the corpus exercising the `list<slug>` type — the one declared non-scalar in the whole grammar. From this commit a stripped or unversioned witness is finding `S6`, a fail-closed coverage regression.
