...
witness: examples/data-architecture-demo/outputs/transport-brief.md
...
`../data-architecture.md` § 4.5 rule 2 names this class entry-bearing and fixes the marker's form: **a fenced `artifact-entry` block carrying the entity key and, since § 4.5.1 amended that rule, one optional `cost:` line — and nothing else**, with the entry's prose untouched. One block per entry, directly under that entry's own `**Stream — …**` line:
...
- **Nothing else enters the marker but the declared cost field.** Origin letter, airport codes, mode, duration, passenger list and every flag stay in the entry's labelled prose, where they already are. § 4.2's frontmatter/body test decides *frontmatter* versus *body*, and this class's frontmatter is file-scoped — one block, the first bytes of the file — so an entry-level value has no field to become. The key does, and so — since `../data-architecture.md` § 4.5.1 amended rule 2 — does one optional `cost:` line. **`cost` used to be named in this sentence** as a value barred from the marker, and its removal is the amendment landing rather than an omission. **The `**Cost:**` prose line is unchanged and stays the master**: it carries both bases — per person and group total — with any caveat the writer attached, while the marker carries one normalized scalar and its declared `basis`, so a stream priced both ways states the basis its marker means. **`agents/04-transport.md` emits the line from this release**, and because this is the one class whose master states both bases it is the one that needed a selection rule. `reference/adr/ADR-018-cost-estimation-method.md` § *Decision 4* fixes it: **`per-person` where the line states a per-person figure, `group-total` only where it does not.** The preference is not stylistic — a `group-total` has to be divided across that stream's own participant set downstream, and a stream whose set cannot be resolved is **not** divided by the roster but lands on a named `unallocated` line instead. **§ *Point-to-Point Transit Matrix* is untouched by any of this**: § 4.5 rules that a secondary table inside a fence-form class carries no marker, so it has nowhere to put a `cost:` line and gains none.
